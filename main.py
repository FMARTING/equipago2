...
def actualizar_total_pagos(equipo_id):
	jugadores = db.GqlQuery("Select * from Jugadores where equipo_id= :1", str(equipo_id)).fetch(None)
	for a in jugadores:
		pagos_jugador = db.GqlQuery("Select * from Pagos where jugador_id= :1", str(a.key().id()))
		total = 0
		for x in pagos_jugador:
			total +=x.monto
		a.total_pagos = total
...
class Jugadores (db.Model):
	...
	total_pagos = db.IntegerProperty(default = 0)
	gastos_inscripcion = db.IntegerProperty(default = 0)
...
class MainPage(Handler):
	...
	def get(self):
		...
		try:
			gasto_mes_jugador = int(equipo.gastos_total/tamano_equipo(equipo_id))
		except:
			gasto_mes_jugador = 0
...
class GastosF (Handler):
	...
	def post(self):
		...
		jugadores = db.GqlQuery("Select * from Jugadores where equipo = %s" %(str(equipo_id)))
		for a in jugadores:
			a.gastos_inscripcion = int(gastos_inscr)/tamano_equipo(equipo_id)
		self.redirect('/main')
